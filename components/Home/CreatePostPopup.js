...
const CreatePostPopup = ({props}) => {
  ...
  const [communityOptionsCoord, setCommunityOptionsCoord] = useState({left: "0px",top: "0px"});
    ...
    return (
      ...
      <div
          ...
          ></div>
            ...
            <div
              className={`flex flex-col h-fit absolute z-10 ${showCommunityOptions ? 'enter-fade-animation' : 'exit-fade-animation '}`}
              ...
              
      {
        joinedCommunities.map(community => {
          ...
          return(
            ...
            }} className="flex flex-row items-center cursor-pointer p-2 m-2 rounded-2xl hover:bg-p-btn" id={community._id} logoImageUrl={community.logoImageUrl}>
              
                <Image src={community.logoImageUrl ? community.logoImageUrl : '/gradient.jpg'} className="rounded-full" width={30} height={30} />
             
              <div className="text-p-text ml-4 text-base " id={community._id} logoImageUrl={community.logoImageUrl}>{community.name}</div>
            </div>
          )
        })
      }
    ...
    const type = files.type.split('/')[0]
        return(
          <div className="flex items-center justify-center">
            ...
            {type === "image" && <img src={imageValue} className="max-h-80 rounded-2xl"  alt="Your amazing post"/>}
            {type === "video" &&  <video src={imageValue} className="max-h-80 rounded-2xl" controls></video>}
            <AiOutlineClose onClick={removeImage} className="text-s-text w-7 h-7 bg-p-bg rounded-full p-1 absolute z-10 top-2 right-2" />
            </div>
          </div>
        )
      }
  ...
  const PopUpModal = () =>{
    return(
      // simple modal
      <PopUpWrapper title="Create Post" onClick={handleSubmit} label="POST" loading={loading}>
            <div className="border border-s-text rounded-full text-p-text ml-3 w-fit px-1">
              ...
              <Image src={showCommunity.image} className="rounded-full" width={30} height={30} />
            ...
            {/* <!-- Modal body --> */}
            <div>
            <FormTextInput label="Title" placeholder="Here you go" value={title} onChange={onChangeTitle} />
                <div className="text-base leading-relaxed  m-4">
                  { files ? showAddedFile() :(
                  <label htmlFor='upload-file'><div className="h-32 text-s-text flex flex-col justify-center items-center border border-s-text  rounded-xl" >
