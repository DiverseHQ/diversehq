...
const Giphy = ({ setGifAttachment }) => {
  ...
  const [gifPosition, setGifPosition] = useState('top')
  const { gifButtonRef } = useRef(null)
  const router = useRouter()

  useEffect(() => {
    //set the gifPosition to bottom if there is no space on top of the screen
    if (router.pathname === '/') {
      setGifPosition('top-right')
      return
    }
    const button = document.getElementById('giphy-button')
    const buttonPosition = button.getBoundingClientRect()
    const popupHeight = 530
    const spaceAboveButton = buttonPosition.top
    const spaceForPopup = spaceAboveButton > popupHeight ? 'top' : 'bottom'
    setGifPosition(spaceForPopup)
  }, [showGiphyModal])

  return (
    <>
      <OptionsWrapper
        showOptionsModal={showGiphyModal}
        setShowOptionsModal={setShowGiphyModal}
        OptionPopUpModal={() => (
          <GifSelector
            setShowModal={(option) => setShowGiphyModal(option)}
